...
async function deleteContainer(containerId: string) {
  if (
    !confirm(
      "Are you sure you want to delete this container and all its items?"
    )
  )
    return;
...
async function moveToLocation(containerId: string, locationId: string) {
  ...
  alert(
    res.ok ? "Container moved to location root!" : "Failed to move container."
  );
}
...
export default function ContainerCard({
  ...
  // Add modal state (preselect both location and container)
  ...
  const collectDescendants = (
    c: ContainerWithDetails | undefined | null,
    set: Set<string>
  ) => {
  ...
  // Normalize allContainers to minimal shape for child components / modal
  const normalized = (
    allContainers as Array<{
      id: string;
      name: string;
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Right-side actions: Add (+) and Delete */}
        <div
          className="flex items-center gap-1 shrink-0"
          onClick={(e) => e.stopPropagation()}
        >
          {/* Add FAB — opens ModalAdd with location + container preselected and locked */}
          <button
            type="button"
            aria-label={`Add in ${container.name}`}
              ...
              <section className="rounded-lg border border-gray-200 bg-white/90 p-1 hover:border-indigo-200 transition-colors">
                ...
                <div className={moveTab === "location" ? "block" : "hidden"}>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      moveToLocation(container.id, e.target.value)
                    }
                ...
                <div className={moveTab === "container" ? "block" : "hidden"}>
                  ...
                  <select
                    defaultValue=""
                    onChange={(e) =>
                      moveIntoContainer(container.id, e.target.value)
                    }
      ...
      {/* Add Modal for this container (location + container preselected & locked) */}
      <ModalAdd
        open={modalOpen}
        onClose={() => setModalOpen(false)}
        selectedType="item"
        selectedLocationId={container.locationId ?? null}
